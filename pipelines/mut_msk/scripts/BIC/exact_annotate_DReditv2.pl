...
my $PERL = '/opt/common/CentOS_7/perl/perl-5.26.1/bin';
my $VCF2MAF = '/home/redmondd/PROGRAMS/vcf2maf-1.6.6';
my $HG19_FASTA = '';
my $B37_FASTA = '/data/wolchok/PROJECT/refs/GRCh37hg19/Homo_sapiens_assembly19.fasta';
my $PICARD = '/home/redmondd/PROGRAMS/PICARD';
my $JAVA = '/opt/common/CentOS_7/java/jdk1.8.0_131/bin/';
my $BCFTOOLS = '/home/redmondd/PROGRAMS/bcftools-1.8';
my $TABIX = '/home/redmondd/PROGRAMS/tabix-master'; 
...
my $annotations = '/home/redmondd/scripts/BIC-variants_pipeline/ExAC.r0.3.sites.vep.vcf.gz';
...
`$JAVA/java -jar $PICARD/picard.jar SortVcf VALIDATION_STRINGENCY=LENIENT INPUT=$strippedFiles OUTPUT=$output/xtra$somatic/sortedSimpleVCF.vcf SEQUENCE_DICTIONARY=$output/ref$somatic/$refDictBase`;
print "$JAVA/java -jar $PICARD/picard.jar SortVcf VALIDATION_STRINGENCY=LENIENT INPUT=$strippedFiles OUTPUT=$output/xtra$somatic/sortedSimpleVCF.vcf SEQUENCE_DICTIONARY=$output/ref$somatic/$refDictBase\n\n";

`sed 's/chr//g;s/ID=chr/ID=/g' $output/xtra$somatic/sortedSimpleVCF.vcf | uniq > tmp123; mv tmp123 $output/xtra$somatic/sortedSimpleVCF.vcf`;

# BGZIP, tabix stuff:
`$TABIX/bgzip -f $output/xtra$somatic/sortedSimpleVCF.vcf`;
`$TABIX/tabix -p vcf $output/xtra$somatic/sortedSimpleVCF.vcf.gz`;
...
#
print "$BCFTOOLS/bcftools annotate --annotations $annotations --columns AC,AN,AF --output-type v --output $output/xtra$somatic/exac.vcf $output/xtra$somatic/sortedSimpleVCF.vcf.gz\n\n";
`$BCFTOOLS/bcftools annotate --annotations $annotations --columns AC,AN,AF --output-type v --output $output/xtra$somatic/exac.vcf $output/xtra$somatic/sortedSimpleVCF.vcf.gz`;
