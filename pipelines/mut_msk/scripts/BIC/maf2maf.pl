...
if( -s $vep_anno ) {
    ...
    $vep_cmd .= " --polyphen b --af --af_1kg --af_esp" if( $species eq "homo_sapiens" );
    ...
    $vep_cmd .= " --plugin ExAC,$vep_data/ExAC.r0.3.1.sites.vep.vcf.gz" if( $species eq "homo_sapiens" and $ncbi_build eq "GRCh37" );
